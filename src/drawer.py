...
class Drawer:
    def __init__(self):
        ...
        self.keyboard_mapping = {
            "s": self.save_drawing, # Press 's' key for saving the path
            "x": self.close_window, # Press 'x' key to close window
            "z": self.undo_movement,# Press 'z' key to undo last movement
            "h": self.show_help,    # Press 'h' key to show help
            "<Up>": self.increase_line_width, # Press 'Up' arrow key to increase line width
            "<Down>": self.decrease_line_width# Press 'Down' arrow key to decrease line width
        }
    ...
    def run(self): # main loop method
        ...
        for key, func in self.keyboard_mapping.items():
